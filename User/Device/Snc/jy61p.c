...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){		//串口中断读取角度信息
...
void JY61P_START(void)
{
    HAL_UART_Transmit(&huart1,JY61P_ULOCK_CMD, sizeof(JY61P_ULOCK_CMD),10000);  //解锁
    delay_ms(200);//延时200ms
    HAL_UART_Transmit(&huart1,JY61P_XY0_CMD, sizeof(JY61P_XY0_CMD),10000);    //XY轴归零
    delay_ms(200);
    HAL_UART_Transmit(&huart1,JY61P_Z0_CMD, sizeof(JY61P_Z0_CMD),10000);      //Z轴归零
    delay_ms(200);
    HAL_UART_Transmit(&huart1,JY61P_SAVE_CMD, sizeof(JY61P_SAVE_CMD),10000);    //保存
    delay_ms(200);
}
